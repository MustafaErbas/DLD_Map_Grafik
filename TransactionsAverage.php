...
<head>
    ...
    <title>Ortalama Fiyat Analizi</title>
...
<body>
    ...
    <div id="map-container">
        ...
        <div class="stats-panel">
            <div class="panel-title">Pazar Özeti</div>

            <div class="stat-card highlight">
                <span class="stat-label">GENEL ORTALAMA</span>
            ...
            <div style="display:flex; gap:10px;">
                <div class="stat-card" style="flex:1;">
                    <span class="stat-label" style="color:#ef4444;">EN YÜKSEK</span>
                    <span class="stat-value" id="stat-high" style="font-size:16px;">...</span>
                    <span class="stat-sub" id="stat-high-name">...</span>
                </div>

                <div class="stat-card" style="flex:1;">
                    <span class="stat-label" style="color:#22c55e;">EN DÜŞÜK</span>
    ...
    <script>
        ...
        // Daha sade, gri tonlu bir harita altlığı (Veri ön plana çıksın diye)
        ...
        // --- YENİ RENK SKALASI (MAVİ VE VARYANSLARI) ---
        // Koyu Mavi (Pahalı) -> Açık Mavi (Ucuz)
        function getColor(price) {
            return price > 30000000 ? '#08306b' :
                price > 20000000 ? '#08306b' :
                price > 10000000 ? '#08306b' :
                price > 5000000 ? '#08519c' :
                price > 3000000 ? '#2171b5' :
                price > 2000000 ? '#4292c6' :
                price > 1500000 ? '#6baed6' :
                price > 1000000 ? '#9ecae1' :
                price > 750000 ? '#c6dbef' :
                price > 500000 ? '#deebf7' :
                price > 0 ? '#f7fbff' : // En Ucuz (Neredeyse Beyaz)
                'transparent';
        }
        ...
        function style(feature) {
            ...
            if (data && data.count > 0) {
                avgPrice = data.total_value / data.count;
            }
            ...
            return {
                ...
                weight: 1,
                opacity: 1,
                color: 'white', // Bölge sınırları beyaz
                fillOpacity: 0.7 // Renkler net görünsün
            };
        }
        ...
        function onEachFeature(feature, layer) {
            ...
            if (data && data.count > 0) {
                ...
                var formattedPrice = fullPriceFormatter.format(avgPrice);

                var tooltipContent = `
                    <div class="tooltip-header">${geoName}</div>
                    <div class="tooltip-body">
                        <div class="tooltip-row"><span>Ortalama Fiyat</span><span class="tooltip-val" style="color:#08519c">${formattedPrice}</span></div>
        ...
        // --- YENİ İSTATİSTİK HESAPLAMA FONKSİYONU ---
        function calculateStats(dataObj) {
            ...
            for (var key in dataObj) {
                ...
                if (item.count > 0) {
                    var avg = item.total_value / item.count;
            ...
            document.getElementById('stat-high').innerText = priceFormatter.format(maxPrice);
            document.getElementById('stat-high-name').innerText = maxName;
            document.getElementById('stat-high-name').title = maxName; // Hover için

            document.getElementById('stat-low').innerText = priceFormatter.format(minPrice === Infinity ? 0 : minPrice);
        ...
        function fetchTransactions() {
            fetch('Api/api_transactions_count.php')
                ...
                .then(data => {
                    ...
                    drawMap();
                    calculateStats(transactionData); // Liste yerine İstatistik Hesapla
                })
        ...
        function drawMap() {
            ...
            legend.onAdd = function(map) {
                ...
                div.innerHTML += '<span class="legend-title">Ortalama Fiyat</span>';

                // Lejant Kısmı (drawMap fonksiyonu içinde legend.onAdd kısmı)
                var grades = [0, 1000000, 1500000, 2000000, 3000000, 5000000, 10000000, 20000000, 30000000];
                var labels = ["< 1M", "1M - 1.5M", "1.5M - 2M", "2M - 3M", "3M - 5M", "5M - 10M", "10M - 20M", "20M - 30M", "> 30M"];
