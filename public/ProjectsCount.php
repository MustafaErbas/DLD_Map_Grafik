...
<head>
    ...
    <title>Dubai Proje Sayıları</title>
...
<body>
    ...
    <div id="map-container">
        ...
        <div class="top-list-panel">
            <div class="panel-header">
                <span>En Çok Proje Olan Bölgeler</span>
            </div>
    ...
    <script>
        ...
        // 4. Kapsamlı İsim Eşleştirme Listesi (Eksik Veri Olmasın Diye)
        var nameMapping = {
        ...
        function getColor(d) {
            return d > 50 ? '#8e44ad' : d > 30 ? '#c0392b' : d > 20 ? '#e74c3c' : d > 10 ? '#e67e22' : d > 5 ? '#f1c40f' : d > 2 ? '#2980b9' : d > 0 ? '#1abc9c' : 'transparent';
        }
        ...
        function onEachFeature(feature, layer) {
            ...
            if (data && data.count > 0) {
                var formattedValue = currencyFormatter.format(data.total_value);
                var tooltipContent = `
                    ...
                    <div class="tooltip-body">
                        ...
                        <div class="tooltip-row"><span>Toplam Değer</span><span class="tooltip-val" style="color:#e67e22">${formattedValue}</span></div>
                    </div>`;
        ...
        function fetchProjects() {
            // Log ekliyorum
            console.log("API'den veri isteniyor...");

            fetch('Api/api_projects.php')
                .then(res => res.json())
                .then(data => {
                    console.log("API Verisi Geldi:", data); // Veri gelip gelmediğini burada göreceğiz
                    ...
                    for (var csvKey in data) {
                        ...
                        if (mapNamesSet.has(targetKey)) {
                            ...
                            // console.log("Eşleşmeyen Bölge:", csvKey); 
                        }
                    }

                    drawMap();
                    updateTopList(projectData);
                    runDiagnosticReport();
                })
                .catch(err => {
                    console.error("API Hatası:", err);
                    document.getElementById('topListContent').innerHTML = '<div style="color:red; padding:20px;">API Hatası: Veri çekilemedi.</div>';
                });
        }
        ...
        function drawMap() {
            ...
            legend.onAdd = function(map) {
                ...
                div.innerHTML += '<span class="legend-title">Proje Sayısı</span>';
                var grades = [1, 2, 5, 10, 20, 30, 50];
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML += '<div class="legend-item"><i style="background:' + getColor(grades[i] + 1) + '"></i><span>' + (grades[i] === 1 ? '1 - 2' : grades[i] + (grades[i + 1] ? ' &ndash; ' + grades[i + 1] : '+')) + '</span></div>';
                }
                return div;
            };
            legend.addTo(map);
        ...
        function runDiagnosticReport() {
            setTimeout(function() {
                ...
                for (var key in projectData) {
                    if (projectData[key].count > 0) {
                        matched += projectData[key].count;
                    }
                }
                console.log(`✅ Haritaya İşlenen Toplam Proje: ${matched}`);
            }, 2000);
        }
