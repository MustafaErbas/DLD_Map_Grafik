...
<head>
    ...
    <title>Dubai Transactions Haritası</title>
...
<body>
    ...
    <div id="map-container">
        ...
        <div class="top-list-panel">
            <div class="panel-header">
                <span>En Çok Satış Yapan Bölgeler</span>
            </div>
            <div class="panel-content" id="topListContent">
                <div style="padding:20px; text-align:center; color:#999; font-size:13px;">Veriler Analiz Ediliyor...</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function normalizeName(name) {
            ...
            clean = clean.replace(/\bIND\b/g, 'INDUSTRIAL');
            clean = clean.replace(/\bRES\b/g, 'RESIDENTIAL');
            clean = clean.replace(/\bFIRST\b|\b1ST\b/g, '1');
            clean = clean.replace(/\bSECOND\b|\b2ND\b/g, '2');
            clean = clean.replace(/\bTHIRD\b|\b3RD\b/g, '3');
            clean = clean.replace(/\bFOURTH\b|\b4TH\b/g, '4');
            clean = clean.replace(/\bFIFTH\b|\b5TH\b/g, '5');
            clean = clean.replace(/\bSIXTH\b|\b6TH\b/g, '6');
            clean = clean.replace(/\bSEVENTH\b|\b7TH\b/g, '7');
            clean = clean.replace("AL BARSHAA", "AL BARSHA");
            clean = clean.replace("AL GOZE", "AL QOUZ");
            clean = clean.replace("UM SUQAIM", "UMM SUQEIM");
            clean = clean.replace("NAD AL", "NADD AL");
            return clean.replace(/\s+/g, ' ').trim();
        }

        // --- BENZERLİK ALGORİTMASI (LEVENSHTEIN) ---
        ...
        function getColor(d) {
            return d > 5000 ? '#8e44ad' : d > 1000 ? '#c0392b' : d > 500 ? '#e74c3c' : d > 200 ? '#e67e22' : d > 100 ? '#f1c40f' : d > 50 ? '#2980b9' : d > 0 ? '#1abc9c' : 'transparent';
        }

        function style(feature) {
            var geoName = feature.properties.CNAME_E || feature.properties.LABEL_E || "";
            var cleanGeoName = normalizeName(geoName);
            var data = transactionData[cleanGeoName];
        ...
        function onEachFeature(feature, layer) {
            ...
            if (data && data.count > 0) {
                ...
                var formattedPrice = new Intl.NumberFormat('en-AE', {
                    style: 'currency',
                    currency: 'AED',
                ...
                var tooltipContent = `
                    ...
                    <div class="tooltip-body">
                        <div class="tooltip-row"><span>Toplam Satış</span><span class="tooltip-val">${data.count} Adet</span></div>
                        <div class="tooltip-row"><span>Ortalama Fiyat</span><span class="tooltip-val" style="color:#e67e22">${formattedPrice}</span></div>
                    </div>`;
        ...
        function updateTopList(dataObj) {
            var list = [];
            for (var key in dataObj) {
                if (dataObj[key].count > 0) list.push({
                    name: key,
                    count: dataObj[key].count
                });
            }
            list.sort((a, b) => b.count - a.count);
            var html = "";
            list.forEach((item, index) => {
                html += `<div class="list-item"><span class="rank">#${index + 1}</span><span class="area-name" title="${item.name}">${item.name}</span><span class="count-badge">${item.count}</span></div>`;
            });
            document.getElementById('topListContent').innerHTML = html;
        }
        ...
        function fetchTransactions() {
            fetch('Api/api_transactions_count.php')
                .then(res => res.json())
                .then(data => {
                    transactionData = {};
                    ...
                    for (var csvKey in data) {
                        ...
                        if (!transactionData[targetKey]) {
                            transactionData[targetKey] = {
                                count: 0,
                                total_value: 0
                            };
                        }
                        transactionData[targetKey].count += record.count;
                        transactionData[targetKey].total_value += record.total_value;
                    }

                    // Veriler Hazır Olduğunda Haritayı Çiz
                    drawMap();
                    updateTopList(transactionData);
                    runDiagnosticReport();
                })
        ...
        function drawMap() {
            ...
            legend.onAdd = function(map) {
                ...
                div.innerHTML += '<span class="legend-title">İşlem Hacmi</span>';
                var grades = [1, 50, 100, 200, 500, 1000, 5000];
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML += '<div class="legend-item"><i style="background:' + getColor(grades[i] + 1) + '"></i><span>' + (grades[i] === 1 ? '1 - 50' : grades[i] + (grades[i + 1] ? ' &ndash; ' + grades[i + 1] : '+')) + '</span></div>';
                }
                return div;
            };
            legend.addTo(map);
        ...
        function runDiagnosticReport() {
            setTimeout(function() {
                console.clear();
                console.log("%c📊 SİSTEM RAPORU", "color: white; background: #2c3e50; padding: 5px;");
                var matched = 0,
                    lost = 0;
                for (var key in transactionData) {
                    if (transactionData[key].count > 0) {
                        if (mapNamesSet.has(key)) matched += transactionData[key].count;
                        else lost += transactionData[key].count;
                    }
                }
                var total = matched + lost;
                var rate = total > 0 ? ((matched / total) * 100).toFixed(2) : 0;
                console.log(`✅ Harita Verisi: ${matched}`);
                console.log(`❌ Kayıp Veri: ${lost}`);
                console.log(`📈 Başarı: %${rate}`);
            }, 2000);
        }
