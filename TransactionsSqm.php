...
<head>
    ...
    <title>m² Birim Fiyat Analizi</title>
...
<body>
    ...
    <div id="map-container">
        ...
        <div class="stats-panel">
            <div class="panel-title">m² Fiyat Analizi (AED)</div>

            <div class="stat-card highlight">
                <span class="stat-label">GENEL ORTALAMA m²</span>
                <span class="stat-value" id="stat-avg">...</span>
                <span class="stat-sub">Tüm Bölgeler Bazında</span>
            </div>

            <div style="display:flex; gap:10px;">
                <div class="stat-card" style="flex:1;">
                    <span class="stat-label" style="color:#08306b;">EN YÜKSEK</span>
                    <span class="stat-value" id="stat-high" style="font-size:16px;">...</span>
                    <span class="stat-sub" id="stat-high-name">...</span>
                </div>

                <div class="stat-card" style="flex:1;">
                    <span class="stat-label" style="color:#6baed6;">EN DÜŞÜK</span>
                    <span class="stat-value" id="stat-low" style="font-size:16px;">...</span>
    ...
    <script>
        ...
        function normalizeName(name) {
            ...
            clean = clean.replace("UM SUQAIM", "UMM SUQEIM").replace("NAD AL", "NADD AL");
        ...
        // --- GÜNCELLENMİŞ m² FİYAT SKALASI (10K - 100K) ---
        function getColor(pricePerSqm) {
            return pricePerSqm > 100000 ? '#021a36' : // Ultra Lüks (100k+)
                pricePerSqm > 75000 ? '#042e5f' : // 75k - 100k
                pricePerSqm > 50000 ? '#08519c' : // 50k - 75k
                pricePerSqm > 40000 ? '#2171b5' : // 40k - 50k
                pricePerSqm > 30000 ? '#4292c6' : // 30k - 40k
                pricePerSqm > 20000 ? '#6baed6' : // 20k - 30k
                pricePerSqm > 15000 ? '#9ecae1' : // 15k - 20k
                pricePerSqm > 10000 ? '#c6dbef' : // 10k - 15k
                pricePerSqm > 0 ? '#f7fbff' : // < 10k (Ucuz)
                'transparent';
        }
        ...
        function style(feature) {
            ...
            var pricePerSqm = 0;
            if (data && data.total_area > 0) {
                pricePerSqm = data.total_value / data.total_area;
            }
            ...
            return {
                fillColor: getColor(pricePerSqm),
                weight: 1,
                opacity: 1,
                color: 'white',
                fillOpacity: 0.85
        ...
        function onEachFeature(feature, layer) {
            ...
            if (data && data.total_area > 0) {
                var pricePerSqm = data.total_value / data.total_area;
                var formattedPrice = priceFormatter.format(pricePerSqm);

                var tooltipContent = `
                    <div class="tooltip-header">${geoName}</div>
                    <div class="tooltip-body">
                        <div class="tooltip-row"><span>m² Birim Fiyatı</span><span class="tooltip-val" style="color:#08519c">${formattedPrice}</span></div>
                        <div class="tooltip-row"><span>Toplam İşlem</span><span class="tooltip-val">${data.count} Adet</span></div>
                    </div>`;
        ...
        function calculateStats(dataObj) {
            ...
            var globalTotalArea = 0;

            var minSqmPrice = Infinity;
            var minName = "-";
            ...
            for (var key in dataObj) {
                ...
                if (item.total_area > 0) {
                    var avgSqm = item.total_value / item.total_area;

                    globalTotalValue += item.total_value;
                    globalTotalArea += item.total_area;

                    if (avgSqm > maxSqmPrice) {
                        maxSqmPrice = avgSqm;
                        maxName = key;
                    }
                    if (avgSqm < minSqmPrice) {
                        minSqmPrice = avgSqm;
                        minName = key;
                    }
                }
            ...
            var globalAvg = globalTotalArea > 0 ? (globalTotalValue / globalTotalArea) : 0;

            document.getElementById('stat-avg').innerText = priceFormatter.format(globalAvg);

            document.getElementById('stat-high').innerText = priceFormatter.format(maxSqmPrice);
            document.getElementById('stat-high-name').innerText = maxName;
            document.getElementById('stat-high-name').title = maxName;

            document.getElementById('stat-low').innerText = priceFormatter.format(minSqmPrice === Infinity ? 0 : minSqmPrice);
                ...
                .then(data => {
                    ...
                    for (var csvKey in data) {
                        ...
                        if (record.total_area) {
                            transactionData[targetKey].total_area += parseFloat(record.total_area);
                        }
                    }
        ...
        function drawMap() {
            ...
            legend.onAdd = function(map) {
                ...
                div.innerHTML += '<span class="legend-title">m² Fiyatı (AED)</span>';

                // 10K - 100K Skalası
                var grades = [0, 10000, 15000, 20000, 30000, 40000, 50000, 75000, 100000];
                var labels = ["< 10K", "10K-15K", "15K-20K", "20K-30K", "30K-40K", "40K-50K", "50K-75K", "75K-100K", "> 100K"];

                for (var i = 0; i < grades.length; i++) {
                    var color = getColor(grades[i] + 100);
                    div.innerHTML +=
                        '<div class="legend-item"><i style="background:' + color + '"></i><span>' + labels[i] + '</span></div>';
                }
                return div;
            };
            legend.addTo(map);
